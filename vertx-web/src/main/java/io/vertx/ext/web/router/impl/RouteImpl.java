...
import io.vertx.ext.web.router.impl.RouterImpl;
...
public class RouteImpl implements Route {
  ...
  private String pathInput; // Used to match if route already exists
  private String path;
  ...
  private List<String> params_names;
  ...
  synchronized boolean matches(RoutingContext context, String mountPoint, boolean failure) {
    ...
    if (failure && !hasNextFailureHandler() || !failure && !hasNextContextHandler()) {
      return false; // No more context handlers
    }
    if (!enabled) {
    ...
    if (!methods.isEmpty() && !methods.contains(request.method())) {
      return false; // No matching methods
    }
        ...
        if (m.groupCount() > 0) {
          ...
          if (params_names != null) {
            ...
            for (int i = 0; i < params_names.size(); i++) {
              final String k = params_names.get(i);
  ...
  private void createPatternRegex(String path) {
    ...
    params_names = new ArrayList<>();
    int index = 0;
    while (m.find()) {
      String param = "p" + index;
      String group = m.group().substring(1);
      if (params_names.contains(group)) {
        throw new IllegalArgumentException("Cannot use identifier " + group + " more than once in pattern string");
      }
      m.appendReplacement(sb, "(?<" + param + ">[^/]+)");
      params_names.add(group);
      index++;
    }
    m.appendTail(sb);
    path = sb.toString();
